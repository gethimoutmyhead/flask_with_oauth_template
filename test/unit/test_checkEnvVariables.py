...
load_dotenv('flaskApp-settings.env')

def checkEnvVariable(varName):
	assert len(os.getenv(varName)) > 0, f"env variable {varName} length <= 0" 
...
is_envVariable_NotEmptyString('authserver_configEnv')
load_dotenv(os.getenv('authserver_configEnv'))
